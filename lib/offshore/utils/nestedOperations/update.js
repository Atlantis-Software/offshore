...
module.exports = function(parents, associations, cb) {

  var self = this;

  // Combine model and collection associations
  associations = _.extend(associations.collections, associations.models);

  // Build up .add and .update operations for each association
  var operations = buildOperations.call(self, parents, associations);
...
function buildOperations(parents, associations) {

  var self = this;
  var operations = {};

  // For each association, grab the primary key value and normalize into model.add methods
  _.keys(associations).forEach(function(association) {

    var optValues = associations[association];
...
function queueOperations(parents, association, operation, values) {
  ...
  var self = this;
  var attribute = self.offshore.schema[self.identity].attributes[association];
  var modelName;
  ...
  var collection = self.offshore.collections[modelName];
...
function updateRunner(parents, operations, cb) {
  ...
  function update(operation, next) {
    var model = self.offshore.collections[operation.model]._loadQuery(self._query);
...
function linkRunner(parents, operations, cb) {
  ...
  function updateParentRecord(parent, next) {
    ...
    var criteria = {};
    var model = self.offshore.collections[self.identity]._loadQuery(self._query);
    ...
    model.update(criteria, pValues).exec(function(err) {
      ...
      parent._loadQuery(self._query).save(next);
    });
  }
...
function buildParentRemoveOperations(parent, operations) {
  ...
  Object.keys(operations).forEach(function(association) {
    ...
    var child = self.offshore.schema[attribute.collection];
 ...
 * operation using offshore collection instances.
 *
...
function removeOperationRunner(operations, cb) {
  ...
  function runner(operation, next) {
    ...
    if (!operation.nullify) {
      self.offshore.collections[operation.model]._loadQuery(self._query).destroy(operation.criteria).exec(next);
      return;
    }
    ...
    self.offshore.collections[operation.model]._loadQuery(self._query).update(operation.criteria, values).exec(next);
  }
...
function createNewRecords(parent, recordsToCreate, cb) {
  ...
  function mapAssociations(association, next) {
    ...
    var model = self.offshore.collections[referencedCollection]._loadQuery(self._query);
